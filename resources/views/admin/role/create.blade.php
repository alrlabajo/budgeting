<x-app-layout>
    <div class="px-6 py-32">
        ...
        <div class="flex justify-between items-center pb-4">
            ...
            <label class="w-96 h-8 absolute text-indigo-800 text-3xl font-extrabold font-['Inter'] leading-9 whitespace-nowrap">Create Role</label>
            </div>

            <div class="flex justify-end">
                <a href="{{ url('roles') }}" class="text-white font-extrabold bg-indigo-800 hover:bg-indigo-900 focus:ring-4 focus:ring-indigo-300 rounded-lg text-sm px-5 py-2.5 dark:bg-indigo-800 dark:hover:bg-indigo-900 focus:outline-none dark:focus:ring-indigo-800">Back</a>
            </div>
        </div>
        <div class="w-full h-full px-10 py-4 bg-white rounded-lg shadow border border-zinc-300 space-y-4">
            <form  wire:submit="submit" action="{{ url('roles') }}" method="POST">
                @csrf
                <div class="px-6 py-2">
