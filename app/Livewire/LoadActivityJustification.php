...
class LoadActivityJustification extends Component
{
    ...
    public function mount($college_office = null, $year = null) {
        $this->load_activityjustification = ActivityJustification::when($college_office, function ($query, $college_office) {
        ...
        $this->year = $this->load_activityjustification->map(function ($activityjustification) {
            return $activityjustification->created_at->format('Y');
        })->unique()->values();
    }
    ...
    {
        return view('livewire.load-activity-justification');
    }
}
